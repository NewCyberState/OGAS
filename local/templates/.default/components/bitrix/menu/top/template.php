...
foreach($arResult as $arItem):?>
	...
	<?else:?>
		...
		<?if ($arItem["PERMISSION"] > "D"):?>
			...
			<?if ($arItem["DEPTH_LEVEL"] == 1):?>
				<li class="nav-item nav-item-submenu <?if ($arItem["SELECTED"]):?>nav-item-expanded nav-item-open bg-primary <?endif?>"><a href="<?=$arItem["LINK"]?>" class="nav-link <?if ($arItem["SELECTED"]):?>nav-item-open<?else:?><?endif?>"><?=$arItem["TEXT"]?></a></li>
			<?else:?>
				<li  class="nav-item"><a href="<?=$arItem["LINK"]?>" class="nav-link <?if ($arItem["SELECTED"]):?> active<?endif?>"><?=$arItem["TEXT"]?></a></li>
			<?endif?>
