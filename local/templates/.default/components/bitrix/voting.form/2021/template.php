...
<div class="card-header bg-white header-elements-lg-inline ">
    <h4 class="card-title font-weight-semibold mb-0">Голосование</h4>
</div>
...
<form action="<?=POST_FORM_ACTION_URI?>" method="post" class="vote-form">
    ...
    <ol class="vote-items-list vote-question-list">
        ...
        foreach ($arResult["QUESTIONS"] as $arQuestion):
            ...
            ?>">
                    ...
                    <?if(true/*!empty($_REQUEST["post_id"])*/):?>



                        <div class="mb-3">
                            <span class="text-muted d-block">Если вы полностью поддерживаете законопроект - проголосуйте "За". Если не поддерживаете - проголосуйте "Против". Если текст законопроекта вам непонятен, понятен не полностью или вам не хвататает компетентности в данном вопросе - <a href="/personal/delegates/adddelegates/">делегируйте</a> свой голос более компетентному в тематике референдума гражданину, либо откажитесь от голосования. Голосуйте ответственно. Изменить свой выбор после голосования будет невозможно. Решение, принятое по результатам голосования будет иметь долгосрочные последствия для вас и всех граждан, кого оно касается.</span>
                        </div>
						...
						<label for="vote_radio_<?=$arAnswer["QUESTION_ID"]?>_<?=$arAnswer["ID"]?>" class="text-danger font-weight-bold mb-1 font-size-lg text-uppercase form-check-label"><input type="radio" <?=$value?> class="form-check-input-styled-primary" name="vote_radio_<?=$arAnswer["QUESTION_ID"]?>" <?
                        ?>id="vote_radio_<?=$arAnswer["QUESTION_ID"]?>_<?=$arAnswer["ID"]?>" <?
                               ?>value="<?=$arAnswer["ID"]?>" <?=$arAnswer["~FIELD_PARAM"]?> />
							<?=$arAnswer["MESSAGE"]?></label>
						</span>
                                            <?
                                            break;
                                        case 1://checkbox?>
    ...
    <?if(true/*!empty($_REQUEST["post_id"])*/):?>
        ...
        <div class="vote-form-box-buttons vote-vote-footer">
            <span><button type="submit" name="vote" class="btn btn-danger btn-lg"  value="<?=GetMessage("VOTE_SUBMIT_BUTTON")?>"><i class='icon-user-check mr-2'></i><?=GetMessage("VOTE_SUBMIT_BUTTON")?></button></span>
