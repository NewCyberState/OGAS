...
else
{
	...
	{
		if(false/*$arResult["is_ajax_post"] != "Y" && $arResult["CanUserComment"]*/)
		{
			...
			<div id="form_comment_" style="display:none;">
				...
				<form method="POST" name="form_comment" id="<?=$component->createPostFormId()?>2" action="<?=$ajaxPath; ?>">
...
//					only for not registered users
		...
		function ShowComment($comment, $tabCount=0, $tabSize=2.5, $canModerate=false, $User=Array(), $use_captcha=false, $bCanUserComment=false, $errorComment=false, $arParams = array())
		{
			...
			if($comment[AUTHOR_ID])
			    $comment["urlToAuthor"] = "/user/".$comment[AUTHOR_ID]."/";
			else
                $comment["urlToAuthor"] = "";

			global $APPLICATION;

			$comment["urlToBlog"] = /*$APPLICATION->GetCurPage()*/"";
			//pr($comment);
...
//							dbg showComment?
				...
				foreach($sArray[$key] as $comment)
				{
					...
					if(!empty($sArray[$comment["ID"]]))
					{
						foreach($sArray[$comment["ID"]] as $key1)
						{
							...
							ShowComment($key1, ($level+1), 2.5, $canModerate, $User, $use_captcha, false/*$bCanUserComment*/, $errorComment, $arParams);

							if(!empty($sArray[$key1["ID"]]))
							{
								RecursiveComments($sArray, $key1["ID"], ($level+2), false, $canModerate, $User, $use_captcha, false/*$bCanUserComment*/, $errorComment, $arSumComments, $arParams);
							}
						}
					}
					if($first)
		...
		if($arResult["is_ajax_post"] != "Y")
		{
			if(false/*$arResult["CanUserComment"]*/)
			{
				$postTitle = "";
				if($arParams["NOT_USE_COMMENT_TITLE"] != "Y")
		...
		if($arResult["is_ajax_post"] != "Y" && $arResult["NEED_NAV"] == "Y")
		{
			for($i = 1; $i <= $arResult["PAGE_COUNT"]; $i++)
			{
				...
				?>
					<div id="blog-comment-page-<?=$i?>"<?if($arResult["PAGE"] != $i) echo "style=\"display:none;\""?>><?RecursiveComments($tmp, $arResult["firstLevel"], 0, true, $arResult["canModerate"], $arResult["User"], $arResult["use_captcha"], false/*$arResult["CanUserComment"]*/, $arResult["COMMENT_ERROR"], $arResult["Comments"], $arParams);?></div>
				<?
			}
		}
		else
			RecursiveComments($arResult["CommentsResult"], $arResult["firstLevel"], 0, true, $arResult["canModerate"], $arResult["User"], $arResult["use_captcha"], false/*$arResult["CanUserComment"]*/, $arResult["COMMENT_ERROR"], $arResult["Comments"], $arParams);
