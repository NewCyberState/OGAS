...
$APPLICATION->IncludeComponent(
	...
	"2021",
	Array(
		"FORM_ID" => $component->createPostFormId()."2",
		"SHOW_MORE" => "Y",
		"PARSER" => blogTextParser::GetEditorToolbar(array('blog' => $arResult['Blog'])),
		"LHE" => array(
			'id' => $component->createEditorId()."2",
...
BX.ready(function() {
	...
	window["UC"]["f<?=$component->createPostFormId()?>2"] = new FCForm({
		entitiesId : {},
		formId : '<?=$component->createPostFormId()?>2',
		editorId : '<?=$component->createEditorId()?>2',
		<?if(isset($switcerhId) && $switcerhId <> '') :?>
		fileDialogSwitcherId : '<?=$switcerhId?>',
		<?endif;?>
		editorName : ''
	});
		if (!!window["UC"]["f<?=$component->createPostFormId()?>2"].eventNode)
		{
			BX.addCustomEvent(window["UC"]["f<?=$component->createPostFormId()?>2"].eventNode, 'OnUCFormClear', __blogOnUCFormClear);
			BX.addCustomEvent(window["UC"]["f<?=$component->createPostFormId()?>2"].eventNode, 'OnUCFormAfterShow', __blogOnUCFormAfterShow);
			BX.addCustomEvent(window["UC"]["f<?=$component->createPostFormId()?>2"].eventNode, 'OnUCFormSubmit', __blogOnUCFormSubmit);
		}
});
</script>