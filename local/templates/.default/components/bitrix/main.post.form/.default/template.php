...
?><ol class="feed-add-post-strings-blocks feed-add-vote-wrap"><?
    ...
    {
        ?><li class="form-group">
        <label><?=GetMessage("MPF_DESTINATION")?></label>
        <?
    ...
    if (!empty($arParams["TAGS"]))
    {
        ...
        ?>
        <li id="post-tags-block-<?=$arParams["FORM_ID"]?>" class="mt-2 border-top-0 form-group"<?if (true/*$tags !== ""*/):?> style="display:block"<?endif?>>
            <label><?=GetMessage("MPF_TAGS")?></label>
            <div class="ui-tile-selector-selector-wrap" id="post-tags-container-<?=$arParams["FORM_ID"]?>">
                <?=$tags?>
...
<div class="form-group">
    <label><?if($arParams[FORM_ID]=="POST_BLOG_COMMENT_FORM" || strstr($arParams[FORM_ID],"REPLIER")):?>Комментарий<?else:?>Описание проблемы<?endif;?></label>


<label class="feed-add-post" id="div<?=$arParams["divId"]?>" <?if($arParams["LHE"]["lazyLoad"]):?> style="display:none;"<? endif; ?>><?
