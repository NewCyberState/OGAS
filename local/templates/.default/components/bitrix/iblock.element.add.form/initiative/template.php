    ...
    <div class="col-lg-12">
        <div class="alert alert-info bg-white alert-styled-left alert-arrow-left p-0">
            <div class="card-header header-elements-inline">
                <h5 class="card-title">Добавить инициативу</h5>
                <button type="button" class="close" data-dismiss="alert"><span>×</span></button>
            </div>

            <div class="card-body" style="">
                Добавьте новую инициативу с помощью формы, расположенной ниже. Привлеките внимание граждан с помощью запоминающегося названия инициативы - того требования, которое вы предлагаете им поддержать. Пишите четко и кратко. Выберите одну или несколько подходящих законодательных тематик из выпадающего списка. Выберите группу граждан, которым адресована инициатива. Только участники выбранной группы увидят инициативу. Подберите подходящее изображение инициативы - это должен быть файл формата JPG или PNG горизонтальной ориентации, размером не менее 1000x500 пикселей. Опишите проблему и расскажите о том, что вы хотите изменить. Расскажите подробно, каким вы видите решение проблемы. При создании новой инциативы придерживайтесь действующего законодательства, не допускайте призывов к незаконным действиям.
        ...
        <div class="card ">
            <div class="">
                ...
                <form name="iblock_add" action="<?= POST_FORM_ACTION_URI ?>" method="post"
                    ...
                    <input type="hidden" name="PROPERTY[IBLOCK_SECTION][]" value="101">
                    <input type="hidden" name="PROPERTY[56][0][VALUE]" value="1">
                        ...
                        <table class="profile-table table table-striped">
                            <? if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])): ?>
                                ...
                                <? foreach ($arResult["PROPERTY_LIST"] as $propertyID): ?>
                                    ...
                                    <? if ($propertyID == "IBLOCK_SECTION" || $propertyID == "56") continue; ?>

                                    <? if ($propertyID == "58"):

//pr($arResult[ELEMENT_PROPERTIES][$propertyID]);
                                        $group_id=$arResult[ELEMENT_PROPERTIES][$propertyID][0]["VALUE"];
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <select name="PROPERTY[58][0]" class="form-control select-search" data-fouc data-placeholder="Выберите группу...">
                                                    <option></option>
                                                    <?
                                                    $i = 0;
                                                    ...
                                                    while ($res = $arRes->Fetch()):
                                                        if (CSocNetGroup::CanUserInitiate($USER->GetID(), $res[ID])):?>
                                                            <option value="<?= $res[ID] ?>" <?if($group_id==$res[ID]) echo "selected";?>><?= $res["NAME"] . " (" . $res["NUMBER_OF_MEMBERS"] . plural_form($res["NUMBER_OF_MEMBERS"], array(" участник", " участника", " участников")) . ")"; ?></option>
                                                        <? endif; ?>
                                                    <? endwhile; ?>
                                                </select>
                                            </td>
                                    ...
                                    <? if ($propertyID == "59"):
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <select name="PROPERTY[59][]" class="form-control multiselect" multiple="multiple" data-fouc
                                                        data-placeholder="Выберите одну или несколько тематик...">

                                                    <?
                                                    $i = 0;
                                                    $arThematics=array();
                                                    foreach ($arResult[ELEMENT_PROPERTIES][$propertyID] as $value)
                                                        $arThematics[]=$value["VALUE"];
                                                    ...
                                                    while ($res = $arRes->Fetch()):
                                                        ?>
                                                        <option value="<?= $res[ID] ?>" <?if(in_array($res[ID],$arThematics)) echo "selected";?>><?= $res["NAME"] ?></option>
                                                    <? endwhile; ?>
                                                </select>
                                            </td>
                                        </tr>
                                                    ...
                                                    for ($i = 0; $i < $inputNum; $i++) {
                                                        ...
                                                        if (!empty($value) && is_array($arResult["ELEMENT_FILES"][$value])) {
                                                            ...
                                                            <input type="checkbox" class="form-check-input"
                                                                   ...
                                                                   value="Y"/>
                                                            <?= GetMessage("IBLOCK_FORM_FILE_DELETE") ?></label></div>
                                                            <?
                            ...
                            <tr>
                                <td colspan="2">
                                    <input type="submit" class="btn btn-primary" name="iblock_submit"
