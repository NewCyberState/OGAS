...
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	<?=bitrix_sessid_post()?>
    <input type="hidden" name="PROPERTY[77][0]" value="<?=$arParams["GROUP_ID"]?>" />
	...
	<table class="table table-borderless table-sm">
		<thead>
			<tr>
                <td colspan="2"><h4 class="mb-0">Загрузить новый документ</h4></td>
			</tr>
		</thead>
		...
		<tbody>
            ...
            <?if($propertyID==77) continue;?>
				<tr>
					<td><label class="col-form-label pt-0"><?if (intval($propertyID) > 0):?>

                            <?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?><span class="starrequired">*</span><?endif?></label>
						<?
						...
						<input type="file" class="form-control h-auto" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>"  name="PROPERTY_FILE_<?=$propertyID?>_<?=$arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] : $i?>" />
									<?
