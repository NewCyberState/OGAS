...
if(count($arResult["POSTS"])>0)
{
	foreach($arResult["POSTS"] as $ind => $CurPost)
	{
        ...
        <div class="card">
            ...
            <div class="card-header <?=$bgcolor?>  text-white d-flex header-elements-inline">
                <h6 class="font-weight-semibold mb-0 card-title text-truncate">
                    <a href="<?=$CurPost["urlToPost"]?>" class="<?=$color?>" title="<?=$CurPost["TITLE"]?>"><?= $CurPost["TITLE"]; ?></a>
                </h6>
                <div class="header-elements">
                    <a href="<?=$CurPost["urlToPost"]?>" title="<?=$CurPost["TITLE"]?>"><span class="badge <?=$badge?> ml-2"><?=$arParams["STATUS_NAME"]?></span></a>
                </div>
            </div>
            ...
            <div class="card-header bg-white d-flex mb-0">
                <ul class="list-inline list-inline-dotted text-muted  mb-0">
                    <li class="list-inline-item">
                        <?//pr($CurPost);?>
                        <span class="blog-post-date-formated"><?if($arParams["STATUS_ID"]==12) {?>Начало:<?}elseif($arParams["STATUS_ID"]==13){?>Отклонен<?}elseif($arParams["STATUS_ID"]==14){?>Принят<?}?>
                        <?=$CurPost[POST_PROPERTIES][DATA][UF_STATUS_DATE][VALUE]?>
                        </span>
                    ...
                    <li class="list-inline-item">
                        <span class="blog-post-day"><span class="blog-post-date-formated">Окончание:
                                <?=date("d.m.Y h:i:s",strtotime("+1 week",MakeTimeStamp($CurPost[POST_PROPERTIES][DATA][UF_STATUS_DATE][VALUE], "DD.MM.YYYY HH:MI:SS")));
                               ?></span>
                    </li>
                    <?}?>
	...
	<?*/?>
            ...
            <div class="card-footer d-flex">
                ...
                <li class="list-inline-item"><a href="<?=$CurPost["urlToPost"]?>#comments" title="Комментариев" alt="Комментариев"><span class="blog-post-link-caption"><i class="icon-comment mr-1 icon-1x"></i></span> <?=IntVal($CurPost["NUM_COMMENTS"]);?></a></li>
                </ul>
